...
class WebsiteExplorerContractTests(unittest.TestCase):
    ...
    def test_market_view_names_book_price_consensus_and_expected_return(self):
        ...
        self.assertIn("function evaluateUpcomingPaperOffers", script)
        self.assertIn("Only one selection per fight receives a hypothetical stake", script)
        self.assertRegex(script, r"window\.setInterval\(\(\) => \{ renderQualifiedUpcomingBets\(\);[^\n]+60 \* 1000\)")
    ...
    def test_all_upcoming_bet_board_is_compact_and_mobile_safe(self):
        ...
        self.assertIn('meanEv < threshold || floorEv <= 0', script)
    ...
    def test_published_bet_performance_has_bankroll_and_timing_controls(self):
        ...
        self.assertIn("Bayesian chance range", script)
        self.assertIn("Allocated paper stake", script)
        self.assertIn("Standard Kelly (uncertainty not included)", script)
        self.assertIn("what the price's break-even rate means", script)
        self.assertIn("candidate-discrete-time-competing-risks-v1", script)
        self.assertIn("outcome_model_evaluation.json", script)
        self.assertIn('staking === "robust_bayesian_kelly"', script)
