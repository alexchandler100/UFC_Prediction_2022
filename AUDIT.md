...
- A separate market-snapshot workflow makes two bounded Thursday attempts, shares the publication concurrency lock, validates the frozen model/card first, stages exactly five market files, and refuses to publish if the starting branch tip changed. Thursday is deliberately early enough for occasional Friday cards when UFCStats provides only a calendar date.
...
3. Let the new Thursday collector accumulate native, timestamped multi-book snapshots. Evaluate market-only versus market-plus-stats on untouched future cards, and report quote/model/decision coverage separately.
