...
- A separate market-snapshot workflow makes nine bounded Thursday-Saturday observations, shares the publication concurrency lock, validates the frozen model/card first, stages only the quote/forecast/source-timing/decision ledgers and bounded report, and refuses to publish if the starting branch tip changed. Once a previously timed card commences, later attempts become successful no-ops without an API request.
...
3. Let the expanded Thursday-Saturday collector accumulate native, timestamped multi-book snapshots. Evaluate market-only versus market-plus-stats on untouched future cards, and report quote/model/decision coverage, paper return, and CLV separately.
...
6. Create a free The Odds API key and store it as the Actions repository secret `THE_ODDS_API_KEY`; never commit it. The maximum ten weekly requests use roughly 87 of the free tier's 500 monthly credits.
